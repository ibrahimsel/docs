...
The Graph Observer runs on the same device as the managed ROS stack. It must not degrade the performance of the workload it monitors.

**CPU impact:** Each probe cycle calls `get_node_names_and_namespaces()` and per-node topic/service introspection. These are DDS discovery queries resolved locally. Measured overhead on a Jetson AGX Orin with 50 active nodes: **< 5ms per probe cycle**. At the default 5-second interval, this is < 0.1% CPU utilization.

**Memory impact:** The `GraphSnapshot` for a 50-node graph is approximately 15 KB serialized. With one snapshot cached (desired baseline) plus one current snapshot, the memory overhead is **< 50 KB**.

**Thread model:** The probe runs on a ROS timer callback in the Composer's executor. It does **not** create additional threads or processes. It does **not** hold locks during introspection. It does **not** use real-time priority &mdash; the observer operates at normal scheduler priority and yields to any RT-scheduled workload nodes.

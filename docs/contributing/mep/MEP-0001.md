...
Instead of requiring operators to enumerate every expected node, Muto uses a **learn from deployment** approach with **multi-sample stabilization** to filter out transient startup nodes:

1. After `ORCHESTRATION_COMPLETED` fires, the Composer begins a stabilization sequence.
2. The Composer takes **N snapshots** (default: 3) at configurable intervals (default: 15 seconds apart) by calling the Daemon's `GetGraphState` service.
3. The desired state baseline is the **intersection** of node FQNs present in **all** N snapshots. Nodes that appear in only some snapshots are logged as "transient" and excluded from the baseline.
4. The stable baseline is published to the Daemon as a `DesiredState` message and persisted at `~/.muto/state/_active/graph_desired.json`.
5. All subsequent drift detection compares the live graph against this learned baseline.
6. If the stack is redeployed (same manifest), the baseline is re-learned after the new deployment succeeds.

:::warning Why Multi-Sample?
A single snapshot taken after a fixed stabilization period is fragile: transient nodes (parameter loaders, one-shot calibration nodes, map publishers that exit after publishing) may still be running at the snapshot time and get learned into the baseline as permanent. When they naturally exit, they appear as drift. Taking the intersection across multiple samples filters these out automatically.
:::
...
sequenceDiagram
    ...
    C->>C: ORCHESTRATION_COMPLETED
    Note over C: Begin multi-sample stabilization
    C->>D: GetGraphState (sample 1, T+15s)
    D->>RG: Probe live graph
    RG-->>D: GraphSnapshot
    D-->>C: {A, B, C, D}
    C->>D: GetGraphState (sample 2, T+30s)
    D-->>C: {A, B, C}
    C->>D: GetGraphState (sample 3, T+45s)
    D-->>C: {A, B, D}
    Note over C: Baseline = intersection = {A, B}
    Note over C: Transient: C, D (logged)
    C->>D: /muto/daemon/desired_state (A, B)
    Note over D: Begin continuous drift detection
    loop Every 5 seconds
        D->>RG: Probe live graph
        D->>D: Compare against baseline {A, B}
        alt Drift detected
...
The stabilization interval is configurable via `graph_observer_stabilization_sec` (default: 15.0) and the number of samples via `graph_observer_stabilization_samples` (default: 3). For `stack/declarative` stacks where the desired state is known from the manifest, stabilization is skipped entirely &mdash; the manifest *is* the desired state.

### Stack Lifecycle Model

Muto follows a **single-active-stack, replacement semantics** model. This is a deliberate architectural constraint that simplifies drift detection, state persistence, and reconciliation.

**Key properties:**

- **One active stack at a time.** Deploying a new stack replaces the previous one. `StatePersistence` tracks a singleton active state at `~/.muto/state/_active/state.json`.
- **The Daemon receives exactly one `DesiredState` at any time.** There is no multi-stack ownership to resolve — the live graph is always compared against a single desired state.
- **Different stack types cannot coexist.** Deploying `stack/native` replaces a running `stack/declarative`. The kill step of the previous stack runs before the new stack is launched.
- **Version transitions archive the previous baseline.** On version change, the current desired state is archived for potential rollback, drift detection pauses, and a new desired state takes over after deployment completes (immediately for `stack/declarative`, after multi-sample stabilization for `stack/workspace` and `stack/native`).
- **Kill action terminates ALL managed launchers** for the active stack, as implemented by `LaunchPlugin.handle_kill()`.

**Implications for drift detection:**

Because there is exactly one active stack, the Daemon does not need to distinguish which nodes belong to which stack. All desired nodes belong to the active stack. External nodes (not part of the active stack) are handled via `ignored_prefixes` in the `DesiredState` message.

:::note Future Consideration
Multi-stack coexistence (e.g., running a perception stack alongside a planning stack independently) is not in scope for this MEP. Supporting it would require ownership tagging per node, multi-baseline tracking in the Daemon, and conflict resolution when stacks share nodes. This is deferred to a future MEP if the use case arises.
:::
...
|---|---|---|---|
| **Muto Daemon** | C++ / rclcpp | `muto_daemon` | **Hot path:** probe graph, load desired state, compute drift, publish snapshots/events/drift, serve `GetGraphState`. |
| **Muto Composer** | Python / rclpy | `muto_composer` | **Cold path:** subscribe to drift events, evaluate reconciliation policy (criticality, retries, cooldowns), execute corrective actions (launch/kill nodes, param set), serve `ReconcileNow`, sync graph state to Ditto twin. |
...
graph LR
    ...
    DSR -->|reads| BL[(desired_state_cache.json)]
    ASP --> DD
    DSR --> DD
    DD --> GSP
    GSP -->|/muto/graph_state| DB
    GSP -->|/muto/graph_state| RE
    GSP -->|/muto/graph_drift| RE
    RE -->|GRAPH_STATE_UPDATED| DT
    RE -->|restart node| LP
    LP --> N1
    RE -->|full redeploy| EB
```
...
- `/muto/daemon/desired_state` &mdash; `muto_msgs/DesiredState` (typed desired state message, published by Composer after deployment or baseline learning)
...
The Composer owns desired state resolution (it has access to manifests, `StatePersistence`, and the learned baseline). After resolving the desired state, it publishes a typed `muto_msgs/DesiredState` message to `/muto/daemon/desired_state`. The Daemon caches this and uses it for drift comparison.

:::info Ownership Boundaries
- **Composer owns:** desired state resolution, `~/.muto/state/` directory, Ditto sync (via `TwinSynchronizer`), `ReconcileNow` service.
- **Daemon owns:** graph probing, drift computation, snapshot/event/drift publishing, its own restart cache at `~/.muto/daemon/`.

The Daemon does **not** sync to Ditto directly (it has no HTTP client). The Daemon does **not** write to `~/.muto/state/`. Ditto sync flows: Daemon publishes `GraphSnapshot` to `/muto/graph_state` &rarr; `TwinSynchronizer` (Composer) subscribes and syncs to Ditto.
:::
...
sequenceDiagram
    ...
    C->>D: /muto/daemon/desired_state (DesiredState msg)
...
The Daemon persists the received desired state to `~/.muto/daemon/desired_state_cache.json` as a read-through cache so it can recover after a restart without waiting for the Composer to re-publish. This is **not** the source of truth &mdash; the Composer's `~/.muto/state/` is authoritative.
...
The `package_name` and `executable` fields are populated from the manifest for `stack/declarative` and `stack/legacy` (with nodes). For `stack/workspace` and `stack/native`, these fields are **empty** in the learned baseline because ROS 2 graph introspection APIs (`get_node_names_and_namespaces()`, `get_publishers_info_by_topic()`) do not expose package or executable metadata. This affects reconciliation strategy &mdash; see [Reconciliation Loop](#4-reconciliation-loop).

The `managed_by_muto` field is populated by the Daemon's `DriftDetector`: actual nodes whose fully-qualified name matches a node in the desired state set are marked `true`; all others are `false`.

**`msg/DesiredState.msg`** &mdash; Typed desired state channel (replaces `std_msgs/String`):
...
bool paused
string[] ignored_prefixes
```

The `paused` field signals the Daemon to skip drift detection during version transitions. The `ignored_prefixes` field lists FQN prefixes (e.g., `/rviz`, `/_`) that should not be flagged as unexpected when they appear in the live graph but are not in the desired set. This prevents false drift from external nodes (visualization tools, bags, developer tooling).
...
The `stack_id` and `stack_version` are separate fields to preserve the existing separation in `StatePersistence` (which tracks `stack_id` and `current_version` independently). External tools should use `stack_version` for version filtering, not parse it from `stack_id`.
...
3. `DriftDetector::detect()` &mdash; Performs **ownership-scoped** drift detection:
   - **Missing nodes:** desired FQNs not found in actual set.
   - **Unexpected nodes:** actual FQNs not in desired set AND not matching any entry in `ignored_prefixes`.
   - System nodes (`rosout`, `parameter_events`, nodes with `_` prefix, `_daemon`) are always filtered from the actual set before comparison.
   - The `managed_by_muto` flag on each actual node is set to `true` if its FQN matches a desired node, `false` otherwise.
   - If the `paused` flag is set in the cached `DesiredState`, drift detection is skipped entirely (returns no drift).
...
- The Daemon does not subscribe to `ORCHESTRATION_*` EventBus events directly (those are Python-internal). Instead, the Composer signals the Daemon by publishing a `muto_msgs/DesiredState` message with `paused: true` during version transitions, and a new desired state (with `paused: false`) when the transition completes.
...
For `stack/workspace` and `stack/native`, the desired state is always determined via the learn-from-deployment strategy (multi-sample stabilization). Static analysis of complex launch files is not attempted because Python launch files with arbitrary logic, deeply nested conditional includes, and runtime-dependent node composition make reliable extraction infeasible.
...
- **`stack/workspace` / `stack/native`**: Individual node restart requires knowing the node's package and executable, which are **not available** in the learned baseline (ROS 2 graph introspection APIs do not expose this metadata). The reconciliation engine always escalates to a full relaunch of the launch file for these stack types.
...
| `graph_observer_stabilization_sec` | `15.0` | Seconds between stabilization samples after deployment |
| `graph_observer_stabilization_samples` | `3` | Number of snapshots to intersect for learn-from-deployment baseline |
...
4. **Version tracking.** The `GraphSnapshot` includes separate `stack_id` and `stack_version` fields (matching the existing separation in `StatePersistence`). External tools should use `stack_version` for version filtering, not parse it from `stack_id`:

```json
{
  "stack_name": "autoware-full-stack",
  "stack_id": "org.eclipse.muto.fleet:autoware",
  "stack_version": "2024.12.10",
  "status": "converged"
}
```
...
```
GET /api/2/search/things?filter=eq(features/graph/properties/stack_version,"2024.12.10")
```
...
### Phase 0: MEP Amendments
Update this MEP document with design fixes identified during pre-implementation review (ownership-scoped drift, typed `DesiredState`, multi-sample stabilization, responsibility boundaries, Traverser removal, stack lifecycle model).

### Phase 1: Message Types, Stack Rename, Native Handler
- New message types in `muto_msgs`: `NodeState`, `GraphSnapshot`, `GraphDrift`, `GraphEvent`, `DesiredState`, `GetGraphState`, `ReconcileNow`
...
- Demo manifests for all stack types (`stack/declarative`, `stack/workspace`, `stack/native`, `stack/legacy`)
- Unit tests: message generation, handler type acceptance, native handler behavior
- Integration test: native stack launch via `launch_testing`

**Deliverable:** All stack types are handled. `stack/native` enables launching locally-installed software. New typed messages are available for Daemon and Composer.

### Phase 2: Daemon (C++ Package)
- New `muto_daemon` package with `ActualStateProber`, `DesiredStateResolver`, `DriftDetector`, `GraphStatePublisher`, `MutoDaemonNode`
- Timer-based probe cycle publishing to `/muto/graph_state`, `/muto/graph_events`, `/muto/graph_drift`
- `GetGraphState` service for on-demand queries
- Ownership-scoped drift detection with `ignored_prefixes` and `paused` support
- Desired state caching at `~/.muto/daemon/desired_state_cache.json` for restart recovery
- Unit tests: gtest for `DriftDetector` and `DesiredStateResolver`
- Integration test: Daemon observation with live nodes via `launch_testing`

**Deliverable:** The Daemon continuously observes the live ROS 2 graph and publishes drift. External tools can subscribe to real-time graph state.

### Phase 3: Composer Integration (Reconciliation)
- `GraphReconciliationManager` in Composer: subscribes to `/muto/graph_drift`, publishes `DesiredState`, serves `ReconcileNow`
- Multi-sample stabilization for `stack/workspace` and `stack/native` baselines
- Stack-type-aware reconciliation (declarative: precise restart, workspace/native: full launch-file relaunch)
- Criticality-gated policy evaluation (safety: alert only, mission: cautious restart, standard: auto restart)
- New `EventType` members: `GRAPH_STATE_UPDATED`, `GRAPH_DRIFT_DETECTED`, `RECONCILIATION_*`, `NODE_*`
- Unit tests: stabilization intersection, criticality policy, escalation behavior
- Integration tests: declarative reconciliation, native learn-from-deployment, version transition via `launch_testing`

**Deliverable:** Muto detects node crashes within one probe interval and the Composer restarts them according to criticality policy. Escalates to full redeploy after max retries.

### Phase 4: External Observability and Twin Integration
- `/muto/graph_state_json` topic for web dashboards (Daemon-side)
- `TwinSynchronizer` extension: subscribe to `GRAPH_STATE_UPDATED`, sync `GraphSnapshot` to Ditto `features/graph/properties`
- `graph` feature in device twin registration
- Daemon restart recovery (load cached desired state on startup)
- Unit tests: twin graph sync
- Integration test: Daemon restart recovery via `launch_testing`

**Deliverable:** Cloud dashboards visualize live ROS graph state via Ditto or direct ROS topic subscription. Daemon survives restarts without data loss.

### Phase 5: Parameter Drift Detection (Optional, Future)
